...
Vimium はフォーカス中の入力欄で押した Esc を横取りして blur だけを行う (1 回目の Esc はスクリプトに届かない)。
そのため Vimium 併用時にパレットを Esc で閉じるには 2 回押す。🚪🚪
blur された後も文字入力・候補の選択・Enter はそのまま効く (キーを押すと入力欄へフォーカスが戻る)。🔁
